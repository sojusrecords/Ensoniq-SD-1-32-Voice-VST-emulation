...
void menu_main::populate()
{
	...
	{
		if (machine().options().plugins() && !machine().manager().lua()->get_menu().empty())
			item_append(_("menu-main", "Plugin Options"), 0, (void *)PLUGINS);

		if (machine().manager().lua()->call_plugin_check<const char *>("data_list", "", true))
			item_append(_("menu-main", "External DAT View"), 0, (void *)EXTERNAL_DATS);
	}

	item_append(menu_item_type::SEPARATOR);

	if (!machine().manager().favorite().is_favorite(machine()))
...
bool menu_main::handle(event const *ev)
{
	...
	if (ev && (ev->iptkey == IPT_UI_SELECT))
	{
		...
		case FAVORITE:
		{
			favorite_manager &mfav = machine().manager().favorite();
