...
int cli_frontend::execute(std::vector<std::string> &args)
{
	...
	m_result = EMU_ERR_NONE;
	// Create a local mame_machine_manager instance instead of using the singleton.
	// This allows multiple instances to run concurrently in the DAW.
	mame_machine_manager *manager = new mame_machine_manager(m_options, m_osd);
