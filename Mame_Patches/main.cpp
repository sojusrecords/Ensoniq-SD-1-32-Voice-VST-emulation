...
// Update the no-op implementations of these functions to include the 
// additional running_machine parameter, matching emu/main.h signatures.
// This breaks the zexall_machine_manager relationship, but it is necessary 
// to ensure it compiles with the new multi-instance architecture.

void emulator_info::draw_user_interface(running_machine& machine) { }

// Add running_machine reference parameter
void emulator_info::periodic_check(running_machine& machine) { }

// Add running_machine reference parameter
bool emulator_info::frame_hook(running_machine& machine) { return false; }

// Add running_machine reference parameter
void emulator_info::sound_hook(running_machine& machine, const std::map<std::string, std::vector<std::pair<const float *, int>>> &) { }
