...
bool menu_cheat::handle(event const *ev)
{
	...
	if (ev->itemref == ITEMREF_CHEATS_ENABLE)
	{
		if ((ev->iptkey == IPT_UI_LEFT) || (ev->iptkey == IPT_UI_RIGHT) || (ev->iptkey == IPT_UI_CLEAR))
		{
			...
			machine().manager().cheat().set_enable(ev->iptkey == IPT_UI_RIGHT || (ev->iptkey == IPT_UI_CLEAR), false);
			changed = true;
		}
	}
	if ((ev->itemref == ITEMREF_CHEATS_RESET_ALL || ev->itemref == ITEMREF_CHEATS_RELOAD_ALL) && ev->iptkey == IPT_UI_SELECT)
	{
		...
		for (auto &curcheat : machine().manager().cheat().entries())
...
{
	const bool empty = machine().manager().cheat().entries().empty();
	...
	if (!empty)
	{
		...
		item_append_on_off(_("Enable Cheats"), machine().manager().cheat().enabled(), 0, (void *)ITEMREF_CHEATS_ENABLE);
		item_append(menu_item_type::SEPARATOR);
