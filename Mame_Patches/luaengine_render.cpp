...
// Add [this] capture to allow access to m_mame_manager
    render_container_type.set_function(
            "draw_box",
            [this] (render_container &ctnr, float x1, float y1, float x2, float y2, std::optional<uint32_t> fgcolor, std::optional<uint32_t> bgcolor)
            {
...
// Add [this] capture to allow access to m_mame_manager
    render_container_type.set_function(
            "draw_line",
            [this] (render_container &ctnr, float x1, float y1, float x2, float y2, std::optional<uint32_t> color)
            {
                x1 = std::clamp(x1, 0.0F, 1.0F);
                y1 = std::clamp(y1, 0.0F, 1.0F);
                x2 = std::clamp(x2, 0.0F, 1.0F);
	...
	render_container_type.set_function(
			"draw_text",
			[this] (render_container &ctnr, sol::this_state s, sol::object xobj, float y, char const *msg, std::optional<uint32_t> fgcolor, std::optional<uint32_t> bgcolor)
			{
