...
// The 24-bit -> 16-bit delay-RAM write rounds toward zero (magnitude
// truncation) rather than toward -infinity (plain arithmetic shift).
//
// Rationale: with floor truncation, every feedback loop through the delay
// RAM (gain g < 1) has fixed points at all negative values |y| < 256/(1-g),
// so reverb tails decay into a frozen, negatively-biased noise pattern
// instead of silence (measured at -58..-60 dBFS on an SD-1, only a reset
// clears it). Recordings of real hardware show the tail decaying smoothly
// into the analog noise floor, which is only guaranteed by toward-zero
// rounding in the DRAM path. 
// Verified against hardware recordings: SD-1 "OMNIVERSE", chorus
// sidebands and reverb tail both match hardware with this rounding.
static inline int16_t dol_24_to_16(int32_t dol24)
{
		...
		if (state == STATE_HALTED) {
			...
			if (ram_p.cycle != RAM_CYCLE_READ) {
				if (ram_p.cycle == RAM_CYCLE_WRITE) {
					...
					uint16_t dol16 = dol_24_to_16(dol[0]);
					if (ram_p.io) {
						// TODO: implement i/o writing.
						// write_io(ram_p.address, dol16);
						LOG_EXEC("  . RAM: writing %x (%d) [of %x (%d)] to I/O address %x\n", dol16, util::sext(dol16, 16), dol[0], util::sext(dol[0], 24), ram_p.address);
					} else {
						dram_w(ram_p.address, dol16);
