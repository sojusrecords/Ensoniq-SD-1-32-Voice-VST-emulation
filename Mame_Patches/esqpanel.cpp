...
INPUT_CHANGED_MEMBER(esqpanel2x40_vfx_device::analog_value_change)
{
	...
	if (channel >= 0 && channel < 8)
		m_panel_analog_input[channel] = (uint16_t)clamped;  // capture user-drag value for the host mirror
	int value = clamped << 6;
	// Pitch bend (0) and mod wheel (2) are sounded by the host plugin via MIDI. Force their analog
	// path to neutral (pitch = 0x200 center, mod = 0x3ff = no mod) so the wheel still moves visually
	// (the ioport field above is what the layout <animate> reads) without doubling the MIDI bend/mod.
	if (channel == 0) value = 0x200 << 6;
	else if (channel == 2) value = 0x3ff << 6;
