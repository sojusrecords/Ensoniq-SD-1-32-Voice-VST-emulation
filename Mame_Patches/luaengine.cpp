...
lua_engine::lua_engine(mame_machine_manager *manager)
    : m_mame_manager(manager) // Initialize the new member
	, m_lua_state(nullptr)
	, m_machine(nullptr)
	, m_timer(nullptr)
{
 ...
 */
	...
	emu["start"] =
		[this](const char *driver)
		{
			...
			if (i != -1)
			{
				m_mame_manager->schedule_new_driver(driver_list::driver(i));
	...
	emu["step"] =
		[this] ()
		{
			m_mame_manager->ui().set_single_step(true);
			machine().resume();
		};
	...
	emu["show_menu"] =
		...
		{
			mame_ui_manager &mui = m_mame_manager->ui();
 ...
 */
	...
	screen_dev_type.set_function(
			"draw_box",
			[this] (screen_device &sdev, float x1, float y1, float x2, float y2, std::optional<uint32_t> fgcolor, std::optional<uint32_t> bgcolor)
			{
				...
				mame_ui_manager &ui(m_mame_manager->ui());
				if (!fgcolor)
	...
	screen_dev_type.set_function(
			"draw_line",
			[this] (screen_device &sdev, float x1, float y1, float x2, float y2, std::optional<uint32_t> color)
			{
				float const sc_width(sdev.visible_area().width());
				float const sc_height(sdev.visible_area().height());
				x1 = std::clamp(x1, 0.0F, sc_width) / sc_width;
				...
				y2 = std::clamp(y2, 0.0F, sc_height) / sc_height;
				if (!color)
					color = m_mame_manager->ui().colors().text_color();
 ...
 */
	...
	sol()["manager"] = std::ref(*m_mame_manager);
	sol()["mame_manager"] = std::ref(*m_mame_manager);
