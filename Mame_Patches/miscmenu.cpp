...
menu_machine_configure::menu_machine_configure(
	...
	, m_sys(info)
	, m_curbios(0)
	, m_was_favorite(machine().manager().favorite().is_favorite_system(*info.driver))
...
menu_machine_configure::~menu_machine_configure()
{
	...
	{
		if (m_want_favorite)
			machine().manager().favorite().add_favorite_system(*m_sys.driver);
		else
			machine().manager().favorite().remove_favorite_system(*m_sys.driver);
	}
...
menu_plugins_configure::~menu_plugins_configure()
{
	...
	file_plugin.puts(machine().manager().plugins().output_ini());
}
...
{
	plugin_options const &plugin_opts = machine().manager().plugins();
