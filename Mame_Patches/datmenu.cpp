	...
	, m_clicked_tab(-1)
{
	...
	if (machine().manager().lua()->call_plugin("data_list", system ? system->driver->name : "", lua_list))
	{
		int count = 0;
		...
		{
			std::string version;
			machine().manager().lua()->call_plugin("data_version", count, version);
	...
	, m_clicked_tab(-1)
{
	...
	std::vector<std::string> lua_list;
	bool const retrieved(machine().manager().lua()->call_plugin("data_list", std::string(m_short).append(1, ',').append(m_list).c_str(), lua_list));
	...
	if (!layout || (layout->width() != width))
	{
		...
		if (!m_items_list.empty())
		{
			if (0 > m_items_list[m_current_tab].option)
				...
				machine().manager().lua()->call_plugin("data", m_items_list[m_current_tab].option, buffer);
		}
