...
public:
	...
	running_machine(const machine_config &config, mame_machine_manager &manager);
	~running_machine();
	...
	// Redefine the manager() method to return mame_machine_manager instead of machine_manager.
    // This provides access to the correct instance in contexts where running_machine is available.
    mame_machine_manager &manager() const { return m_manager; }
...
private:
	...
	// Redefine m_manager to be of type mame_machine_manager instead of machine_manager
    mame_machine_manager & m_manager;
