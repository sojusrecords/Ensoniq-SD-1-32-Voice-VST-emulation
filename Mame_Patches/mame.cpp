...
{
    // Utilizing machine.manager() instead of the static instance
    machine.manager().lua()->on_periodic();
}

bool emulator_info::frame_hook(running_machine& machine)
{
    // Utilizing machine.manager() instead of the static instance
    return machine.manager().lua()->frame_hook();
}

void emulator_info::sound_hook(running_machine& machine, const std::map<std::string, std::vector<std::pair<const float *, int>>> &sound)
{
    // Utilizing machine.manager() instead of the static instance
   machine.manager().lua()->on_sound_update(sound);
}


// Add running_machine& parameter and replace mame_machine_manager::instance() with machine.manager()
void emulator_info::layout_script_cb(running_machine& machine, layout_file &file, const char *script)
{
    // TODO: come up with a better way to pass multiple arguments to plugin
    // Use machine.manager() instead of the static instance to support multiple VST3 instances
    //machine.manager().lua()->call_plugin_set("layout", std::make_tuple(&file, script->get_value()));
    
    // Utilize machine.manager() to access the lua engine and sol state
    auto &lua(machine.manager().lua()->sol());
