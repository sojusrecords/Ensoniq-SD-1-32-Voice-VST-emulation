...
public:
	...
	static void periodic_check(running_machine& machine);
	static bool frame_hook(running_machine& machine);
	static void sound_hook(running_machine& machine, const std::map<std::string, std::vector<std::pair<const float *, int>>> &sound); // Can't use sound_stream::sample_t sadly
	static void layout_script_cb(running_machine& machine, layout_file &file, const char *script);
